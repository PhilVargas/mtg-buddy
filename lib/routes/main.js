Pages = new Meteor.Pagination(Cards, {
  ...
  routeSettings(route){
    if (Object.keys(route.params.query).length > 0) {
      ...
      if (Number.isInteger(parseInt(params.minCmc))) {
        query.$and.push({ cmc: { $gte: +params.minCmc } });
      }

      if (Number.isInteger(parseInt(params.maxCmc))) {
        query.$and.push({ cmc: { $lte: +params.maxCmc } });
      }
